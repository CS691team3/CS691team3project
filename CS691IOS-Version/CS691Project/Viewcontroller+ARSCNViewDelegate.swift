...
//
                //Introduce Right virtual Content
                self.displayDetailView1(on: mainNode, xOffset: physicalWidth)
    ...
    func displayDetailView(on rootNode: SCNNode, xOffset: CGFloat) {
        ...
        detailNode.geometry?.firstMaterial?.diffuse.contents = UIImage(named: "arrowblue")
    ...
    func displayDetailView1(on rootNode: SCNNode, xOffset: CGFloat) {
        let detailPlane = SCNPlane(width: xOffset, height: xOffset * 1.4)
        detailPlane.cornerRadius = 0.25
        
        let detailNode = SCNNode(geometry: detailPlane)
        detailNode.geometry?.firstMaterial?.diffuse.contents = UIImage(named: "pacey")
    ...
    func displayWebView(on rootNode: SCNNode, xOffset: CGFloat) {
        ...
        DispatchQueue.main.async {
            let request = URLRequest(url: URL(string: "https://www.youtube.com/channel/UCu-lSTmmwtX0hBssuNcb2Uw")!)
